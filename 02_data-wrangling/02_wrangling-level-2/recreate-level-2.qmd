...
first_three_letters <- function(x) { # Define a function that takes the first three letters of a string (ignore space) and change it to upper case
  str_to_upper(str_sub(str_replace_all(x, " ", ""), 1, 3))
}
gm.wrangled <- gm.wrangled%>% 
  mutate(country_abbrev = first_three_letters(country), # Generate the variable "country_abbrev" by taking the first three letters of the "country" variable
...
#| label: play-around-continent

gm.optional <- gm.wrangled %>%
  mutate(continent_collapse = fct_collapse(continent,
                                  Europe_America = c("European Region", "Americas Region"),  
                                  Asia = "Asian Region",                  
                                  Oceania = "Oceania Region",                
                                  Africa = "African Region")) # Collapse the levels of the "continent" variable (to meet the requirements of objective 12)

gm.optional <- gm.optional%>%
  mutate(continent_abbbrev = first_three_letters(continent)) # Generate the variable "continent_abbbrev" by taking the first three letters of the "continent" variable (to meet the requirements of objective 7)
